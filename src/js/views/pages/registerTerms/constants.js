export const MAIN_TERMS = {
    tactical: [
        {
            text: "Rumah Produksi mengirimkan atau mengajukan film harus disertai dengan proposal film di setiap filmnya (bisa mengajukan lebih dari 1 film tapi tidak lebih dari 3 film) yang akan siap ditayangkan selama periode program berlangsung selama Oktober-November 2021.",
        },
        {
            text: "Rumah Produksi yang berhak mendaftar telah berbadan usaha dalam bentuk Perseroan Terbatas (PT) yang kepemilikan sahamnya dimiliki oleh Warga Negara Indonesia (WNI), atau badan hukum Indonesia, dan terdaftar di Kementerian Hukum dan HAM Republik Indonesia, sudah berdiri sebelum tanggal 1 Januari 2019 yang dibuktikan dengan akta pendirian badan usaha;",
        },
        {
            text: "Rumah Produksi  wajib menyampaikan secara resmi nama pemegang saham, nama dewan komisaris, nama dewan direksi, dan nama eksekutif penanggung jawab perusahaan, dimana tidak terjadi pergantian anggota nama pemegang saham, nama dewan komisaris/dewan direksi, dan nama eksekutif penanggung jawab perusahaan setelah tanggal 1 Januari 2021.",
        },
        {
            text: "Pihak yang mendaftarkan Rumah Produksi tersebut adalah orang yang secara hukum sah mewakili Rumah Produksi sesuai dengan Akta dan/atau AD/ART Rumah Produksi/selevel direksi dan merupakan Warga Negara Indonesia yang dibuktikan dengan kepemilikan Kartu Tanda Penduduk;",
        },
        {
            text: "Penanggung Jawab Rumah Produksi cakap secara hukum (berusia minimal 21 tahun atau sudah menikah sebelum berusia 21 tahun) yang dibuktikan dengan Kartu Tanda Penduduk (KTP), merujuk pada Pasal 330 KUH Perdata, dan:",
            children: [
                "a.	tidak sedang menjalani hukuman (dibuktikan dengan Surat Keterangan dari Pengadilan);",
                "b.	berjiwa sehat/berakal sehat (dibuktikan dengan Surat Keterangan Sehat)."
            ]
        },
        {
            text: "Wajib memiliki Nomor Induk Berusaha (NIB) yang terdaftar pada sistem OSS (Online Single Submission) – Kementerian Investasi/BKPM (Badan Koordinasi Penanaman Modal) atas nama Rumah Produksi sekaligus sebagai Tanda Daftar Usaha Perfilman (TDUP) dan wajib memiliki Tanda Pemberitahuan Pembuatan Film (TPPF) yang masih berlaku, yang dibuktikan dengan melampirkan Salinan NIB dan/atau TDUP serta TPPF;",
        },
        {
            text: "Wajib memiliki rekening di bank atas nama Rumah Produksi dengan status aktif (bukti salinan nomor rekening akan diminta saat Rumah Produksi lolos menjadi calon Penerima Bantuan);",
        },
        {
            text: "Wajib melampirkan salinan kartu Nomor Pokok Wajib Pajak (NPWP) atas nama Badan Usaha Rumah Produksi dan salinan Surat Pemberitahuan Tahunan (SPT) Pajak 1 tahun terakhir;",
        },
        {
            text: "Rumah Produksi wajib menandatangani Surat Pernyataan Tanggung Jawab Mutlak (LAMPIRAN 2 Surat Pernyataan Tanggung Jawab Mutlak);",
        },
        {
            text: "Rumah Produksi mengisi surat permohonan Bantuan Pemerintah Bagi Promosi Taktikal Film Indonesia yang ditujukan kepada Deputi Bidang Ekonomi Digital dan Produk Kreatif c.q. Direktorat Musik, Film dan Animasi (LAMPIRAN 3 Format Surat Permohonan Bantuan Pemerintah bagi Promosi Taktikal Film Indonesia);",
        },
        {
            text: "Rumah Produksi wajib melampirkan Surat Pernyataan Kebenaran Data dan memberikan dokumen yang benar dan bersedia mempertanggungjawabkannya secara hukum baik secara perdata maupun pidana jika data yang diberikan tidak benar (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak pernah, tidak sedang dan tidak akan melakukan pengajuan dan/atau menerima bantuan sejenis dari pemerintah pusat maupun daerah, dan/atau Badan Usaha Milik Negara (BUMN) (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak dalam keadaan Pailit dan/atau dalam keadaan penundaan kewajiban pembayaran hutang (“PKPU”) (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak sedang berada dalam kondisi perkara pengadilan baik secara perdata maupun pidana (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi yang terpilih sebagai Penerima Bantuan Pemerintah Bagi Promosi Taktikal Film Indonesia tidak diperkenankan menerima Dana Bantuan Pemerintah Bagi Produksi Film Indonesia (LAMPIRAN 8 Format Surat Pernyataan Tidak Menerima Bantuan Pemerintah Bagi Produksi Film Indonesia);",
        },
        {
            text: "Bersedia mengikuti proses seleksi dan wajib menandatangani surat pernyataan menerima hasil seleksi dan tidak akan menggugat hasil keputusan Tim Kurator dalam bentuk apapun (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Semua dokumen pendukung, dokumen legalitas, dan dokumen permohonan harus digabungkan satu dokumen berbentuk PDF dengan urutan yang benar, lengkap dengan halaman muka yang merujuk dengan jelas nama Rumah Produksi dan disertai dengan profil legalitas Rumah Produksi (LAMPIRAN 5 Ringkasan Profil Pengusul Proposal Permohonan Bantuan Pemerintah Bagi Promosi Taktikal Film Indonesia);",
        },
        {
            text: "Hak Kekayaan Intelektual tetap dimiliki oleh Rumah Produksi, karena dana Penanggulangan Ekonomi Nasional Perfilman merupakan bantuan pemerintah dalam keadaan Coronavirus Disease 2019. Pihak Kemenparekraf/Baparekraf dan/atau instansi negara lainnya tetap dapat menggunakan materi dan konten promosi pada kegiatan-kegiatan kementerian/lembaga/instansi pemerintah non komersial;",
        },
        {
            text: "Rumah Produksi direkomendasikan untuk menggunakan jasa konsultan keuangan untuk memastikan berkas pertanggungjawaban administrasi dan keuangan lengkap dan sesuai dengan aturan yang ada dalam pemerintahan;",
        },
    ],
    production : [
        {
            text: "Rumah Produksi mengirimkan 1 (satu) proposal film yang telah siap produksi atau film yang sedang dalam tahap penyelesaian produksi di tahun 2021 pada saat melakukan pendaftaran.",
        },
        {
            text: "Rumah Produksi yang berhak mendaftar telah berbadan usaha dalam bentuk Perseroan Terbatas (PT) yang kepemilikan sahamnya dimiliki oleh Warga Negara Indonesia (WNI), atau badan hukum Indonesia, dan terdaftar di Kementerian Hukum dan HAM Republik Indonesia, sudah berdiri sebelum tanggal 1 Januari 2019 yang dibuktikan dengan akta pendirian badan usaha;",
        },
        {
            text: "Rumah Produksi  wajib menyampaikan secara resmi nama pemegang saham, nama dewan komisaris, nama dewan direksi, dan nama eksekutif penanggung jawab perusahaan, dimana tidak terjadi pergantian anggota nama pemegang saham, nama dewan komisaris/dewan direksi, dan nama eksekutif penanggung jawab perusahaan setelah tanggal 1 Januari 2021. ",
        },
        {
            text: "Pihak yang mendaftarkan Rumah Produksi tersebut adalah orang yang secara hukum sah mewakili Rumah Produksi sesuai dengan Akta dan/atau AD/ART Rumah Produksi/selevel direksi dan merupakan Warga Negara Indonesia yang dibuktikan dengan kepemilikan Kartu Tanda Penduduk;",
        },
        {
            text: "Penanggung Jawab Rumah Produksi cakap secara hukum (berusia minimal 21 tahun atau sudah menikah sebelum berusia 21 tahun) yang dibuktikan dengan Kartu Tanda Penduduk (KTP), merujuk pada Pasal 330 KUH Perdata, dan:",
            children: [
                "a.	tidak sedang menjalani hukuman (dibuktikan dengan Surat Keterangan dari Pengadilan);",
                "b.	berjiwa sehat/berakal sehat (dibuktikan dengan Surat Keterangan Sehat)."
            ]
        },
        {
            text: "Wajib memiliki Nomor Induk Berusaha (NIB) yang terdaftar pada sistem OSS (Online Single Submission) – Kementerian Investasi/BKPM (Badan Koordinasi Penanaman Modal) atas nama Rumah Produksi sekaligus sebagai Tanda Daftar Usaha Perfilman (TDUP) dan wajib memiliki Tanda Pemberitahuan Pembuatan Film (TPPF) yang masih berlaku, yang dibuktikan dengan melampirkan Salinan NIB dan/atau TDUP serta TPPF;",
        },
        {
            text: "Wajib memiliki rekening di bank atas nama Rumah Produksi dengan status aktif (bukti salinan nomor rekening akan diminta saat Rumah Produksi lolos menjadi calon Penerima Bantuan);",
        },
        {
            text: "Wajib melampirkan salinan kartu Nomor Pokok Wajib Pajak (NPWP) atas nama Badan Usaha Rumah Produksi dan salinan Surat Pemberitahuan Tahunan (SPT) Pajak 1 tahun terakhir;",
        },
        {
            text: "Rumah Produksi wajib menandatangani Surat Pernyataan Tanggung Jawab Mutlak (LAMPIRAN 2 Surat Pernyataan Tanggung Jawab Mutlak);",
        },
        {
            text: "Rumah Produksi mengisi surat permohonan Bantuan Pemerintah Bagi Promosi Taktikal Film Indonesia yang ditujukan kepada Deputi Bidang Ekonomi Digital dan Produk Kreatif c.q. Direktorat Musik, Film dan Animasi (LAMPIRAN 3 Format Surat Permohonan Bantuan Pemerintah bagi Promosi Taktikal Film Indonesia);",
        },
        {
            text: "Rumah Produksi wajib melampirkan Surat Pernyataan Kebenaran Data dan memberikan dokumen yang benar dan bersedia mempertanggungjawabkannya secara hukum baik secara perdata maupun pidana jika data yang diberikan tidak benar (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak pernah, tidak sedang dan tidak akan melakukan pengajuan dan/atau menerima bantuan sejenis dari pemerintah pusat maupun daerah, dan/atau Badan Usaha Milik Negara (BUMN) (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak dalam keadaan Pailit dan/atau dalam keadaan penundaan kewajiban pembayaran hutang (“PKPU”) (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi tidak sedang berada dalam kondisi perkara pengadilan baik secara perdata maupun pidana (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Rumah Produksi yang terpilih sebagai Penerima Bantuan Pemerintah Bagi Produksi Film Indonesia tidak diperkenankan menerima Dana Bantuan Pemerintah Promosi Taktikal Film Indonesia (LAMPIRAN 8 Surat Pernyataan Tidak Menerima Bantuan Pemerintah Promosi Taktikal Film Indonesia);",
        },
        {
            text: "Bersedia mengikuti proses seleksi dan wajib menandatangani surat pernyataan menerima hasil seleksi dan tidak akan menggugat hasil keputusan Tim Kurator dalam bentuk apapun (LAMPIRAN 1 Surat Pernyataan Rumah Produksi);",
        },
        {
            text: "Semua dokumen pendukung, dokumen legalitas, dan dokumen permohonan harus digabungkan satu dokumen berbentuk PDF dengan urutan yang benar, lengkap dengan halaman muka yang merujuk dengan jelas nama Rumah Produksi dan disertai dengan profil legalitas Rumah Produksi (LAMPIRAN 4 Ringkasan Profil Pengusul Proposal Permohonan Bantuan bagi Produksi Film Indonesia);",
        },
        {
            text: "Hak Kekayaan Intelektual tetap dimiliki oleh Rumah Produksi, karena dana Penanggulangan Ekonomi Nasional Perfilman merupakan bantuan pemerintah dalam keadaan Coronavirus Disease 2019. Pihak Kemenparekraf/Baparekraf dan/atau instansi negara lainnya tetap dapat menggunakan materi dan konten promosi pada kegiatan-kegiatan kementerian/lembaga/instansi pemerintah non komersial;",
        },
        {
            text: "Rumah Produksi direkomendasikan untuk menggunakan jasa konsultan keuangan untuk memastikan berkas pertanggungjawaban administrasi dan keuangan lengkap dan sesuai dengan aturan yang ada dalam pemerintahan;",
        },
    ]
}

export const LOWER_TERMS = {
    tactical: [
        {
            text: "Film yang didaftarkan adalah film berkualitas yang siap tayang dan dapat membangkitkan keinginan penonton untuk menonton film Indonesia di berbagai media penayangan;"
        },
        {
            text: "Film yang didaftarkan harus dalam tahap ‘picture lock’. Artinya adalah adegan-adegan dari hasil syuting/pengambilan gambar sudah diedit, tersusun rapi, dan struktur editingnya tidak akan dirubah lagi;"
        },
        {
            text: "Film sudah lulus dari lembaga sensor film Indonesia;"
        },
        {
            text: "Film sudah siap tayang dan telah memiliki jadwal tayang yang dibuktikan dengan surat kontrak dengan media penayangan atau surat pernyataan bahwa film tersebut akan ditayangkan pada periode Oktober-November 2021. (LAMPIRAN 4 Surat Pernyataan Film Yang Diajukan Telah Memiliki Jadwal Tayang Rilis dan Media Penayangannya);"
        },
        {
            text: "Film untuk siap tayang, Produser harus memberikan filmnya dalam format DCP (Digital Cinema Package) dan juga high resolution MP4;"
        },
        {
            text: "Konten film merujuk pada UU no. 33 tahun 2009 tentang perfilman dan tidak mengandung adegan pornografi, kekerasan, radikal (sara) dan hal-hal lain yang dilarang dalam peraturan perundangan tersebut;"
        },
        {
            text: "Proposal harus memuat informasi dasar sebagai berikut (LAMPIRAN 6 Sistematika Proposal Bantuan Pemerintah bagi Promosi Taktikal Film Indonesia):",
            children: [
                "a.	HALAMAN MUKA: Judul Film untuk Tayang/Film Siap Tayang ",
                "b.	INFORMASI DASAR: Judul, Durasi Film, Tahun Produksi, Genre, Nama Penulis, Nama Sutradara, Nama Produser, Daftar Nama Pemain Utama dan Pendukung, dan platform atau media penayangan filmnya. Status Film: (wajib) picture lock;",
                "c.	Logline (3 kalimat) dan Sinopsis (200 – 400 kata);",
                "d.	Ulasan tentang Siapa Target Penonton (max 400 kata);",
                "e.	Director’s Statement (200-400 kata);",
                "f.	Visi Produser (200-400 kata);",
                "g.	Rencana promosi dan konten promosi dari filmnya;",
                "h.	Rencana anggaran biaya (RAB);",
                "i.	Teaser Poster atau Poster atau still photo yang dapat menggambarkan filmnya;",
                "j.	Target Penyelesaian pekerjaan dalam jangka waktu pelaksanaan program tahun 2021;"
            ]
        },
        {
            text: "Semua dokumen pendukung proposal harus digabungkan ke dalam proposal utama dalam satu dokumen berbentuk PDF dengan urutan yang benar, lengkap dengan halaman muka yang merujuk dengan jelas judul masing-masing film."
        },
    ],
    production: [
        {
            text: "Film Siap Produksi atau Film Dalam Tahap Penyelesaian Produksi merupakan film mencakup semua genre film termasuk juga di dalamnya Film Animasi yang produksinya siap diselesaikan pada tahapan rough cut, pre-visualization atau picture lock di tahun 2021;"
        },
        {
            text: "Konten film merujuk pada UU no. 33 tahun 2009 tentang perfilman dan tidak mengandung adegan pornografi, kekerasan, radikal (sara) dan hal-hal lain yang dilarang dalam peraturan perundangan tersebut;"
        },
        {
            text: "Film yang diusulkan wajib selesai hingga tahap rough cut, pre-visualization atau picture lock pada akhir periode program Bantuan Pemerintah Bagi Produksi Film Indonesia;"
        },
        {
            text: "Proposal harus memuat informasi dasar sebagai berikut (Lampiran 5 Sistematika Proposal Bantuan Pemerintah bagi Produksi Film Indonesia):",
            children: [
                "a.	Halaman muka: judul, film untuk produksi.",
                "b.	Informasi dasar: judul, durasi film, genre, nama penulis, nama sutradara, nama produser, nama animator, daftar nama pemain utama dan pendukung dan target penonton. ",
                "c.	Logline (3 kalimat) dan Sinopsis (200 – 400 kata);",
                "d.	Director’s treatment (200-400 kata).",
                "e.	Visi Produser (200-400 kata);",
                "f.	Estimasi total budget (lihat contoh).",
                "g.	Jadwal, Timeline produksi dan Target Penyelesaian pekerjaan dalam jangka waktu pelaksanaan program tahun 2021. ",
                "h.	Profil para produser atau rumah produksi yang terlibat (200-400 kata).",
                "i.	Profil sutradara (200-400 kata).",
                "j.	Daftar rekanan kerja sama: rental, sound studio, hotel, semua instansi/perusahaan terkait.",
                "k.	Informasi produksi: estimasi jumlah tenaga kerja (kru dan pemain), lokasi pembuatan film.",
                "l.	 Tautan daring dari contoh hasil karya sutradara, produser dan/atau animator"
            ]
        },
        {
            text: "Semua dokumen pendukung proposal harus digabungkan ke dalam proposal utama dalam satu dokumen berbentuk PDF dengan urutan yang benar, lengkap dengan halaman muka yang merujuk dengan jelas judul masing-masing film."
        },
    ]
}
